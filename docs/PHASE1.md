...
26.08.1, KDE Frameworks 6.30.0. Read with `rpm -q` on the upgraded pilot on 2026-09-18.
Two habits follow from getting this wrong once. Query the package manager rather than the running
shell, because `plasmashell --version` aborts without a display and that is exactly the condition
when reading over ssh. And read the machine after updating it, not as installed: the install media
put this pilot on Plasma 6.6.4 with Gear 25.12.3, and every one of those numbers moved once
`dnf upgrade` ran. The version a given capture was taken against is recorded per key in the pilot's
provenance record, because an installed machine can sit behind its own repository indefinitely and
nothing about the desktop makes that visible.
...
| P1-R04 | **Plasma version skew between the pilot and Alpine.** The updated pilot carries Plasma 6.7.5 with KDE Gear 26.08.1; Alpine v3.24 carries Plasma 6.6.6 with KDE Gear 26.04.2. The pilot is ahead on both, so a key captured here may simply not exist on the target yet. A second direction bites in practice: the pilot can also sit behind its own repository, as this one did at 6.6.4 until it was upgraded, so the version under a capture is a fact to read rather than infer from the release. Pilot read 2026-09-18, Alpine 2026-09-18. |
...
| P1-R11 | **A Look-and-Feel package freezes every key it sets.** Plasma copies the package contents/defaults into the user account on first login, and that copy then outranks `/etc/xdg` permanently. Any key delivered that way stops following the image the moment someone logs in once, which is the opposite of what P1-D08 promises. | High | The two mechanisms are kept disjoint. The package carries only the wallpaper and the default containment, both consumed once at account creation; every theming and behaviour key is delivered through `/etc/xdg` alone. Verified by clearing the per-user copy and confirming it is not recreated |
| P1-R12 | **A captured baseline is only valid against the Plasma version it was taken on.** Used across an upgrade, a shortcut delta reports every binding upstream changed as though Tundra had customised it. The pilot demonstrated this: a baseline from 6.6.4 produced sixteen lines of delta on 6.7.5 that nobody had touched. | Medium | The baseline records its Plasma version, and a delta taken against a different one is treated as contaminated. Re-capturing means reverting to a pre-apply snapshot, upgrading, and capturing before `/etc/xdg` is seeded, because a seeded machine has no stock account left to read |
